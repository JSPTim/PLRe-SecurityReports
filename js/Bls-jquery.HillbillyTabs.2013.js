...
//alert($("#MSOLayout_InDesignMode").val());
//$("#tabsContainer").html("");
//}
//function xx()
//{
            ...
            else
            {
                $("h2.ms-webpart-titleText").each(function()
                {
                    $(this).find("span:contains('"+title+"')").each(function()
                    {
                        ...
                        // var newitemurl = $(webPart).find(".newitemurl").attr("data-url");
                        
                        var newButtonPlaceHolder = "";
                        if (title != "" )
                        {
                        	
                        	var newButtonPlaceHolder = '<span  id="bls-tab-'+title.replace(" ","-").toLowerCase()+'" style="float: right;">​</span>';
                        }
      
                         $("#"+ulID).append('<li><a href="#Tab'+index+CEWPID+'" id="TabHead'+index+CEWPID+'" onclick="HillbillyTabClick(this.id);">'+
                                title+'</a>'+newButtonPlaceHolder+'</li>').after('<div id="Tab'+index+CEWPID+'"></div>');
